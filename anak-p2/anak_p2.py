import numpy as np
import pandas as pd
import plotly.express as px
import streamlit as st
from streamlit_gsheets import GSheetsConnection

st.set_page_config(page_title="Dashboard", layout="wide")
st.title(":blue[Data Anak-Anak Persatuan 2 KDW]", text_alignment="center")

conn_anak = st.connection("gsheets_anak", type=GSheetsConnection, ttl=120)
data_anak = conn_anak.read()
#data_anak_usia = data_anak[['NAMA', 'JALAN PERSATUAN', 'USIA (TAHUN)']]
jalan_saring = data_anak['JALAN PERSATUAN'].sort_values().dropna().unique()
#jalan_saring = data_anak_usia['JALAN PERSATUAN'].sort_values().dropna().unique()
jalan_widget = st.multiselect(":material/home: Lokasi Tinggal:", jalan_saring)
if jalan_widget:
    data_anak = data_anak[data_anak['JALAN PERSATUAN'].isin(jalan_widget)]
st.dataframe(data_anak)
with st.expander("Sebaran Usia"):
    usia_kolom = pd.DataFrame(data_anak['USIA (TAHUN)'])
    usia_hitung = usia_kolom['USIA (TAHUN)'].value_counts().reset_index()
    usia_hitung.rename(columns={'count': 'JUMLAH ANAK'}, inplace=True)
    usia_grafik = px.bar(usia_hitung, x='JUMLAH ANAK', y='USIA (TAHUN)', color='USIA (TAHUN)', text_auto=True, orientation='h', height=1000, width=150)
    usia_grafik.update_xaxes(showticklabels=False, title_font_size=20)
    usia_grafik.update_yaxes(tickfont_size=20, title_font_size=20)
    usia_grafik.update_layout(xaxis_title="JUMLAH")
    usia_grafik.update_traces(textangle=0, textposition='outside', textfont_size=20, textfont_weight=500, textfont_color='blue')
    st.plotly_chart(usia_grafik)
